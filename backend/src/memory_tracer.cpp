...
void MemoryTracer::heapAlloc(const std::string& label, int size, const std::string& content) {
    // Rimuovi eventuale blocco precedente con stesso label (riallocazione)
...
MemorySnapshot MemoryTracer::snapshot() const {
    return {stack_, heap_};
}
